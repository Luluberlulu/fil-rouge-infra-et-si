...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Connexion réussie : Le JWT est maintenant stocké en HTTP-Only !
        ...
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-indigo-600/30 transform active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex justify-center"
            >
              {loading ? (
